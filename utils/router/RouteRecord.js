class RouteRecord {
    ...
    #validateOptions(options, index) {
      ...
      if (!options || typeof options !== "object") {
        throw new Error(
          `Invalid RouteRecord${indexStr}: constructor expects an object.`,
        );
      }
      if (typeof options.name !== "string") {
        throw new Error(
          `Invalid RouteRecord${indexStr}: 'name' must be a string.`,
        );
      }
      if (typeof options.path !== "string" || !options.path.startsWith("/")) {
        throw new Error(
          `Invalid RouteRecord${indexStr}: 'path' must be a string starting with '/'.`,
        );
      }
      if (!["string", "function"].includes(typeof options.template)) {
        throw new Error(
          `Invalid RouteRecord${indexStr}: 'template' must be a string or a function.`,
        );
      }
    }
    ...
    #getParamNames(path) {
      const paramNames = (path.match(/(:\w+)/g) || []).map((paramName) =>
        paramName.substr(1),
      );
      return paramNames;
    }
